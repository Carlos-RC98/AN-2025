...
Las **Series temporales**son una sucesión de observaciones registradas a intervalos de tiempo regulares. Estas son fundamentales en modelado predictivo y análisis de datos en diversas disciplinas. 


```{python}
inegi_data = pd.read_excel(r"C:/Users/Ch4rl/Downloads/inegi_data.xlsxl")
...
La media muestral es el promedio de los valores de la serie temporal a lo largo del tiempo. Se calcula:
...
La varianza muestral cuantifica la dispersión de los valores de la serie alrededor de su media. Se calcula:
...
Cuantifica la covarianza entre dos valores de la serie separados por un retraso (lag) $k$. Se calcula:
...
Es una versión normalizada de la autocovarianza, mide la correlacion entre dos valores de la serie separados por un retraso $k$, se calcula:
...
plt.figure(figsize=(10,6))
plt.plot(inegi_diff, label="Diferenciación de la Serie", color= "red")
...
plt.figure(figsize=(10,6))
plt.plot(inegi_ar_res.index, inegi_ar_res["inegi"], label="Original", color="blue", alpha=0.8)
...
```{python}
plt.figure(figsize=(10,6))
plt.plot(inegi_ar_model.resid, label="Residuos", color="red")
plt.title("Residuos del Modelo Autoregresivo")
plt.xlabel("Periodo")
plt.ylabel("Residuos")
plt.axhline(0, color="purple", linestyle="--", linewidth=1, alpha=0.5)
plt.legend()
plt.grid(True, linestyle="--", alpha=0.8)
plt.show()
```
